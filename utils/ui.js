...
export function updateCardValues(power, tariffType) {
    ...
    COMPANIES.forEach(company => {
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            if (providerNameElement && providerNameElement.textContent.includes(company)) {
                card = cards[i];
                break;
            }
        }
        ...
        if (card) {
            ...
            else if (tariffType === 'biHorario' && TARIFF_VALUES[company]?.biHorario) {
            ...
            else if (tariffType === 'triHorario' && TARIFF_VALUES[company]?.triHorario) {
            ...
            powerDetail.innerHTML = `
                ...
                <span class="detail-value">${powerValue.toFixed(4)} €/kVA/dia</span>
            `;
...
export function displayResults(results, calculationDays, energyDiscount, gasDiscount) {
    ...
    let html = `
        <div class="w-full">
            ...
            <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-electric">
                ...
                ${energyDiscount > 0 ? `<p class="text-gray-700 mt-2"><i class="fas fa-percentage mr-1"></i> Desconto aplicado em energia: ${energyDiscount}%</p>` : ''}
                ${gasDiscount > 0 ? `<p class="text-gray-700 mt-2"><i class="fas fa-percentage mr-1"></i> Desconto aplicado em gás: ${gasDiscount}%</p>` : ''}
            </div>
    ...
    results.forEach((result, index) => {
        html += `
            <div class="result-card mb-4 p-4 rounded-lg border ${index === 0 ? 'border-savings bg-green-50' : 'border-gray-200 bg-white'}">
                <div class="flex justify-between items-center mb-1">
                    <div class="flex items-center">
                        <span class="font-bold text-gray-800">${result.company}</span>
                        ${index === 0 ? '<span class="ml-2 px-2 py-1 bg-savings text-white text-xs rounded-full">Melhor Opção</span>' : ''}
                    </div>
